...
-- ============================================================================
-- Weighted vehicle spawning system
-- Higher weight = more likely to spawn. 0 = never spawn.
-- Vehicles not in this table won't spawn as traffic.
-- Mod vehicles that aren't installed are silently skipped.
-- ============================================================================
...
local function invalidateVehiclePool()
  validatedPool = nil
  validatedPoolTotalWeight = 0
end

local function buildValidatedPool() -- validates which models exist and builds the weighted pool
  if validatedPool then return end

  validatedPool = {}
  validatedPoolTotalWeight = 0

  local modelList = core_vehicles.getModelList()
  if not modelList or not modelList.models then
    log('W', logTag, 'Could not get vehicle model list for traffic pool')
    return
  end

  for model, weight in pairs(vehicleWeights) do
    if weight > 0 and modelList.models[model] then
      table.insert(validatedPool, {model = model, weight = weight})
      validatedPoolTotalWeight = validatedPoolTotalWeight + weight
    elseif weight > 0 and not modelList.models[model] then
      log('I', logTag, 'Traffic pool: skipping unavailable model: ' .. model)
    end
  end

  log('I', logTag, string.format('Traffic pool built: %d models, total weight %.0f', #validatedPool, validatedPoolTotalWeight))
end

local function pickWeightedModel() -- picks a random model from the validated pool based on weights
  if not validatedPool or #validatedPool == 0 or validatedPoolTotalWeight <= 0 then
    return defaultData.traffic.model
  end

  local roll = random() * validatedPoolTotalWeight
  local cumulative = 0
  for _, entry in ipairs(validatedPool) do
  ...
  return validatedPool[#validatedPool].model -- fallback
end

local function getRandomConfig(model) -- picks a random non-police config for a model
  local modelData = core_vehicles.getModel(model)
  if not modelData or not modelData.configs then
    return nil
  end
  ...
  for configKey, configData in pairs(modelData.configs) do
    ...
    if not configType or (string.lower(configType) ~= 'police' and string.lower(configType) ~= 'service') then
      table.insert(validConfigs, configKey)
    end
  end

  if #validConfigs == 0 then
    -- fallback: allow any config
    for configKey, _ in pairs(modelData.configs) do
      table.insert(validConfigs, configKey)
    end
  end

  if #validConfigs > 0 then
    return validConfigs[random(#validConfigs)]
  end
  return nil
end
...
local function buildWeightedTrafficGroup(amount) -- builds a traffic group using weighted random selection
  ...
  local group = {}
  local modelCounts = {} -- track how many of each model we've picked to add some variety

  for i = 1, amount do
    -- Try a few times to avoid too many duplicates
    local bestModel = pickWeightedModel()
    local bestCount = modelCounts[bestModel] or 0

    for attempt = 1, 3 do
      local candidate = pickWeightedModel()
      local candidateCount = modelCounts[candidate] or 0
    ...
    local config = getRandomConfig(bestModel)
    local entry = {model = bestModel}
    if config then
      entry.config = config
    end
...
local function swapVehicleModel(id) -- swaps a traffic vehicle to a different weighted model
  local veh = traffic[id]
  if not veh or not veh.isAi then return end
  if veh.role and veh.role.name == 'police' then return end -- don't swap police vehicles

  buildValidatedPool()
  if not validatedPool or #validatedPool == 0 then return end

  local newModel = pickWeightedModel()
  local obj = getObjectByID(id)
  if not obj or newModel == obj.jbeam then return end -- skip if same model

  local config = getRandomConfig(newModel)
...
local function spawnTraffic(amount, groupData, options) -- spawns and processes a group array of vehicles to use as traffic
  ...
  groupData = groupData or buildWeightedTrafficGroup(amount)
  ...
  if not trafficGroup then -- if predefined vehicle group does not exist, create it
    ...
    if amount >= 1 then
      if not trafficGroup then
        trafficGroup = buildWeightedTrafficGroup(amount - policeAmount)
        log('I', logTag, string.format('Built weighted traffic group with %d vehicles', #trafficGroup))
      end
...
local function setupCustomTraffic(amount, params) -- spawns a group of vehicles for traffic, with custom parameters
  ...
  spawnTraffic(amount, buildWeightedTrafficGroup(amount))
end
